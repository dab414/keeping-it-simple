...
The purpose of this document is to run the analyses needed for Appendix B in text, namely fitting a non-linear model and investigating observations at `point_difference > 0` in Experiment 2 data.
...
```{r}
d <- read.csv('../data/disconnect_green_temp.csv')
d
```
...
#l1_pos_twodown <- glmer(transcode ~ difference + (1 | subject), data = pos, family = binomial(link = 'logit'), control = glmerControl(optimizer = 'bobyqa'), nAGQ = 1)
# save(l1_pos, l1_pos_nocov, l1_pos_twodown, file = 'models/l1_pos.RData')
load(file = 'models/l1_pos.RData')
anova(l1_pos, l1_pos_nocov)
anova(l1_pos_nocov, l1_pos_twodown)
```

**Inference**

```{r}
summary(l1_pos_nocov)
exp(fixef(l1_pos_nocov))
exp(confint(l1_pos_nocov, method = 'Wald'))
```

**Hypothesis Testing**

```{r}
#l1_pos_nodiff <- glmer(transcode ~ 1 + (1 | subject) + (0 + difference | subject), data = pos, family = binomial(link = 'logit'), control = glmerControl(optimizer = 'bobyqa'), nAGQ = 1)
